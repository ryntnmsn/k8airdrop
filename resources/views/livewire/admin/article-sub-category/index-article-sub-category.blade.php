<div>
    <div class="flex">
        <div class="flex-1 flex flex-col space-y-1">
            <x-title>News Sub Categories</x-title>
                    ...
                    @foreach ($articlesubCategories as $articlesubCategory)
                        <tr class="border-b hover:bg-slate-100 rounded-xl">
                            ...
                            <td scope="row" class="px-6 py-3 font-medium whitespace-nowrap">
                                {{$articlesubCategory->title}}
                            </td>
                            <td class="px-6 py-3 flex justify-end">
                                <x-button wire:click="editArticleSubCategory({{ $articlesubCategory->id }})" class="!p-2 !bg-transparent !border-0 !text-slate-600" data-modal-target="edit-modal" data-modal-toggle="edit-modal">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-5 h-5">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" />
                                      </svg>
                                </x-button>
                                <x-button wire:click="deleteArticleCategory({{ $articlesubCategory->id }})" data-modal-target="delete-modal" data-modal-toggle="delete-modal" class="!p-2 !bg-transparent !border-0 !text-slate-600" >
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-5 h-5">
...
<div wire:ignore.self id="add-modal" data-modal-backdrop="add" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="relative p-4 w-full max-w-2xl max-h-full">
        ...
        <div class="relative bg-white rounded-lg shadow">
            ...
            <!-- Modal body -->
            <form wire:submit="storeArticleSubCategory">
                <div class="p-4 md:p-5 space-y-4">
                    ...
                    <div>
                        <x-label>Parent Category</x-label>
                ...
                <div class="flex items-center p-4 md:p-5 border-t border-slate-200 rounded-b">
                    <x-button wire:target="storeArticleSubCategory">Create</x-button>
                </div>
            </form>
        </div>
    </div>
...
<div wire:ignore.self id="edit-modal" data-modal-backdrop="edit" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="relative p-4 w-full max-w-2xl max-h-full">
        ...
        <div class="relative bg-white rounded-lg shadow">
            ...
            <!-- Modal body -->
            <form wire:submit="updateArticleSubCategory">
                <div class="p-4 md:p-5 space-y-4">
                    <div>
                        <x-label>Title</x-label>
                    ...
                    <div>
                        ...
                        <x-select wire:model="categoryID">
                            @foreach ($articleCategories as $articleCategory)
                                <option  value="{{ $articleCategory->id }}" {{ $articleCategory->id == $categoryID ? 'selected' : '' }}>{{ $articleCategory->title }}</option>
                            @endforeach
                        </x-select>
                ...
                <!-- Modal footer -->
                <div class="flex items-center p-4 md:p-5 border-t border-slate-200 rounded-b">
                    <x-button wire:target="updateArticleSubCategory">Create</x-button>
                </div>
            </form>
        </div>
    </div>
