<p class="font-semibold text-slate-500 mb-10">{{ __('MENU') }}</p>
            <ul class="flex flex-col space-y-8">
                <li class="{{ request()->is('user/dashboard') ? 'bg-[#a38e5c]' : ''}} pr-10 pl-2 py-3 rounded-full">
                    <a wire:navigate href="{{ route('user.dashboard') }}" class="text-slate-100 font-semibold flex gap-2">
                        <span>
                ...
                {{-- <li class="{{ request()->is('user/spin-the-wheel') ? 'bg-indigo-600' : ''}} pr-10 pl-2 py-3 rounded-full">
                    <a wire:navigate href="{{ route('spin.wheel.dashboard') }}" class="text-slate-100 font-semibold flex gap-2">
                        <span>
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-5 h-5">
                ...
                <li class="{{ request()->is('user/account') ? 'bg-[#a38e5c]' : ''}} pr-10 pl-2 py-3 rounded-full">
                    <a wire:navigate href="{{ route('user.account') }}" class="text-slate-100 font-semibold flex gap-2">
                        <span>
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-5 h-5">
                ...
                <li class="pr-10 pl-2 py-3">
                    <form action="{{ route('user.logout') }}" method="post">
                        @csrf
                        <button type="submit" class="text-slate-100 font-semibold flex gap-2">
                            <span>
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-5 h-5">
